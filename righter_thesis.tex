...
recursion.  For the negative binomial distribution,
\[
    (n+1)\,P_r(N^(s) = n+1) = E[N]\, P_{r+s}(N^(s+1) = n);
\]
...
\[
    (n+1)\,P_r(N^(s) = n+1) = E[N]\, P_{r+0}(N^(s+1) = n).
\]
Substituting the appropriate identity for each component, we have
\begin{align*}
P(M^{(0)} - 1 = n)
    &= \frac{1}{E[N^{(0)}]}
       \sum_{j=1}^{t}\beta_j^{(0)}\, E[N_j^{(0)}] \cdot P_{r_j}(N^{(1)} = n) \\
    &= \sum_{j=1}^{t}
       \frac{\beta_j^{(0)}\, E[N_j^{(0)}]}{E[N^{(0)}]} \cdot P_{r_j}(N^{(1)} = n),
\end{align*}
...
\[
    E[N^{(k+1)}]
    = \sum_{n=0}^{\infty} n \sum_{j=1}^{t}\beta_j^{(k+1)}\,P_{r_j}(N^{(k+1)} = n)
...
P(M^{(k+1)} - 1 = n)
    &= \frac{1}{E[N^{(k+1)}]}
       \sum_{j=1}^{t}\beta_j^{(k+1)}\, E[N_j^{(k+1)}] \cdot P_{r_j}(N^{(k+2)} = n) \\
    &= \sum_{j=1}^{t}
       \frac{\beta_j^{(k+1)}\, E[N_j^{(k+1)}]}{E[N^{(k+1)}]} \cdot P_{r_j}(N^{(k+2)} = n).
\end{align*}
