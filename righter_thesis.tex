%% ===============================================================
%%  Robert Righter – Master's Thesis in AMS Style
%%  Recursive Computation with Finite-Mixture Count Distributions
%% ===============================================================
...
\setlength{\headsep}{18pt}   % try 18pt–24pt

%% Extra breathing room around display-math blocks
\setlength{\abovedisplayskip}{14pt plus 4pt minus 4pt}
\setlength{\belowdisplayskip}{14pt plus 4pt minus 4pt}
...
\titlespacing*{\section}{0pt}{4.5ex plus 1.5ex minus .2ex}{2.5ex plus .5ex}
\titlespacing*{\subsection}{0pt}{3.5ex plus 1ex minus .2ex}{2.0ex plus .5ex}

%% ------ TOC fix: titlesec writes \numberline to .toc but amsart expects \tocsection;
%% provide a 2em indent so the section number does not overlap the first title letter.
\setcounter{tocdepth}{2}
\makeatletter
\renewcommand{\l@section}{\@tocline{1}{0pt}{1pc}{2em}{}}
\renewcommand{\l@subsection}{\@tocline{2}{0pt}{2pc}{3em}{}}
\makeatother

\AtEndEnvironment{proof}{\vspace{1.5\baselineskip}}

\AtBeginEnvironment{definition}{\vspace{2.0\baselineskip}\begin{spacing}{1.5}}
\AtEndEnvironment{definition}{\end{spacing}\vspace{1.5\baselineskip}}
\AtBeginEnvironment{example}{\vspace{2.0\baselineskip}\begin{spacing}{1.5}}
\AtEndEnvironment{example}{\end{spacing}\vspace{1.5\baselineskip}}
\AtBeginEnvironment{theorem}{\vspace{2.0\baselineskip}\begin{spacing}{1.5}}
\AtEndEnvironment{theorem}{\end{spacing}\vspace{1.5\baselineskip}}
\AtBeginEnvironment{corollary}{\vspace{2.0\baselineskip}\begin{spacing}{1.5}}
\AtEndEnvironment{corollary}{\end{spacing}\vspace{1.5\baselineskip}}
\AtBeginEnvironment{proposition}{\vspace{2.0\baselineskip}\begin{spacing}{1.5}}
\AtEndEnvironment{proposition}{\end{spacing}\vspace{1.5\baselineskip}}
...
%% ------ Title Block ------
\title[Recursive Computation with Finite-Mixture Count Distributions]{Recursive Computation of Compound Random Variables with a Finite-Mixture Count Distribution}
...
{\LARGE Recursive Computation of Compound Random Variables with a Finite-Mixture Count Distribution\par}
\vspace{0.14\textheight}
{\Large Robert Righter\par}
\vspace{0.10\textheight}
{\Large June 2026\par}
\vspace{0.8em}
{\Large Master's Thesis\par}
\vspace{0.8em}
{\Large Supervising Professor: Dr. John Whitaker\par}
\vspace{0.8em}
{\Large Department of Mathematics,\par}
\vspace{0.4em}
{\Large Shawnee State University\par}
\vfill
...
\begin{titlepage}
\thispagestyle{empty}
\noindent The thesis entitled ``Recursive Computation of Compound Random Variables with a
Finite-Mixture Count Distribution'' presented by Robert Righter, a candidate for the degree of
Master of Science in Mathematics, has
been approved and is worthy of acceptance.

\vspace{3.5em}
\noindent\begin{minipage}[t]{0.42\textwidth}
\rule{0.95\linewidth}{0.4pt}\\[-0.2em]
Date
\end{minipage}%
\hfill
...
Compound random variables can be computed directly via the law of total probability by
conditioning on the value of $N$ and convolving the claim size distribution, as described in
Ross \cite{ross2010}. However, starting in 1981, alternative approaches were identified that
utilize recursive algorithms to compute compound random variables more efficiently.

\subsection{The Panjer Recursion}

In 1981, Panjer introduced a recursive method for the computation of compound random
variables when the counting distribution satisfies a particular recurrence relation
\cite{panjer1981}. Panjer showed that if the counting distribution $N$ satisfied the recurrence
relation,
\[
    P(N = n) = \left(a + \frac{b}{n}\right) \cdot P(N = n - 1), \quad
    \text{where } n \in \mathbb{Z},\, n > 0,\, a, b \text{ are constants,}
\]
then $P(S_N = k)$ can be computed with a recursive formula. This eliminated the need to
perform explicit convolutions and provided a simpler path for computing $P(S_N = k)$ when
the criteria are met.

Panjer's work was extended by Sundt and Jewell, also in 1981 \cite{sundt-jewell1981}. They
proved that only three distribution families satisfy Panjer's recurrence relation, namely, the
Poisson, Binomial, and Negative Binomial distribution, also called the $(a, b, 0)$ class. They
also introduced a relaxed form of the recurrence relation in which the relation holds for
$n = 2, 3, \ldots$ instead of $n = 1, 2, 3, \ldots$, allowing a different initial value at $n = 0$.
This enabled Panjer's recursive calculations to be extended to counting distributions such as
the truncated negative binomial and related zero-modified distributions, collectively called the
$(a, b, 1)$ class.

Subsequent work by Willmot sought to extend the Panjer framework to accommodate
counting distributions formed by mixing members of the $(a, b, 0)$ class. In 1993, Willmot
derived recursive computation methods for mixed Poisson distributions in which the Poisson
parameter is itself a random variable drawn from a continuous mixing distribution
\cite{willmot1993}. In certain cases, such as the Poisson-Gamma mixture which yields the
Negative Binomial, the resulting marginal distribution remains within the Panjer class and
therefore satisfies the required recurrence relation. However, this approach is limited to
continuous mixtures that collapse into a single known distribution. It does not extend to
discrete finite mixtures where the counting distribution is a weighted combination of
distributions with fixed parameters, as weighted combination distributions do not satisfy
the required recurrence relation.

Panjer recursion remains the standard tool for computation of compound random variables
in actuarial science \cite{klugman2012}. However, the method is limited to a specific set of
counting distributions that satisfy the recurrence relations.

\subsection{The Compound Random Variable Identity}

In the 2003 eighth edition of his textbook \emph{Introduction to Probability Models}, Ross
introduced an identity for compound Poisson random variables and used it to derive a
recursive formula for computing their probability mass function \cite{ross2010}. Building on
this, Pek\"{o}z and Ross published a paper in 2004 providing a probabilistic proof of the
general identity and applying it to derive recursive formulas for compound random variables
when the counting distribution is Poisson, Binomial, Negative Binomial, hypergeometric,
logarithmic, or negative hypergeometric \cite{pekoz-ross}. This approach introduces a
size-biased auxiliary random variable $M$, defined as
\[
    P(M = n) = \frac{n\, P(N = n)}{E[N]}, \quad n = 1, 2, 3, \ldots
\]
Unlike the Panjer recursion, this approach does not require the counting distribution to
satisfy a specific recurrence relation. However, it does introduce a nuance of recursive
depth wherein the distribution of $M - 1$ must be determined at each level of recursion. For
the Panjer-class distributions, the distribution of $M - 1$ at each recursion level can be
expressed in closed form. In the Poisson case, it is identical to the original distribution; in
the Binomial and Negative Binomial cases, it is the same family with a shifted parameter.
Pek\"{o}z and Ross applied the identity to each of these distributions individually but did not
explore the case where the counting distribution is a finite mixture.
...
\[
    S_N = \sum_{i=1}^{N} X_i
\]
is a \emph{compound random variable} \cite{ross2010}.
\end{definition}
...
\begin{theorem}[Compound Random Variable Identity {\cite{ross2010, pekoz-ross}}]
...
\begin{corollary}[{\cite{pekoz-ross}}]\label{cor:main}
...
\addcontentsline{toc}{subsection}{Example~\theexample: N in the Poisson Distribution}
Let $N$ be a random variable of the Poisson distribution with parameter $\lambda = 3$.
So the probability mass function for the distribution of $N^{(s)}$ at recursion level $s$ is
\[
    P^{(s)}(N = n) = \frac{e^{-\lambda}\,\lambda^n}{n!}, \qquad n = 0, 1, 2, \ldots
\]
We claim that $E[N^{(s)}] = \lambda$, where $E[N^{(s)}]$ is the expected value of the
distribution of $N$ at the level of recursion $s$.
...
Before applying Corollary~1 to compute $P(S_N = 4)$, consider an alternate,
more intuitive approach.  This method utilizes the law of total probability
and directly sums the probability of every outcome that yields the target sum.

Recall the \emph{law of total probability}: to calculate $E[X]$ we may take
a weighted average of the conditional expected value of $X$ given $Y = y$,
each term $E[X \mid Y = y]$ being weighted by $P(Y = y)$ \cite[p.~107]{ross2010}.  Written formally,
\[
...
\begin{proof}
\begin{align*}
\sum_{y} E[X \mid Y = y] \cdot P(Y = y)
    &= \sum_{y}\sum_{x} x\,P(X = x \mid Y = y) \cdot P(Y = y) \\
    &= \sum_{y}\sum_{x} x \cdot \frac{P(X = x,\,Y = y)}{P(Y = y)} \cdot P(Y = y) \\
    &= \sum_{y}\sum_{x} x \cdot P(X = x,\,Y = y) \\
    &= \sum_{x} x \sum_{y} P(X = x,\,Y = y) \\
    &= \sum_{x} x \cdot P(X = x) \;=\; E[X]. \qedhere
\end{align*}
\end{proof}

Applying this formula to our specific example, we must determine all of the
possible ways for the sum to equal $4$.  The full list of possibilities is:

\medskip
When $N = 1$, we must have $X_1 = 4$.

When $N = 2$, we must have $(X_1 = 2,\,X_2 = 2)$, or $(X_1 = 1,\,X_2 = 3)$,
or $(X_1 = 3,\,X_2 = 1)$.

When $N = 3$, we must have $(X_1 = 2,\,X_2 = 1,\,X_3 = 1)$,
or $(X_1 = 1,\,X_2 = 2,\,X_3 = 1)$, or $(X_1 = 1,\,X_2 = 1,\,X_3 = 2)$.
...
We know that
\begin{align*}
    P(N = 1) &= \frac{e^{-3}\,3^{1}}{1!} = e^{-3}\cdot 3, \\
    P(N = 2) &= \frac{e^{-3}\,3^{2}}{2!} = e^{-3}\cdot\frac{9}{2}, \\
    P(N = 3) &= \frac{e^{-3}\,3^{3}}{3!} = e^{-3}\cdot\frac{27}{6}, \\
    P(N = 4) &= \frac{e^{-3}\,3^{4}}{4!} = e^{-3}\cdot\frac{81}{24}.
\end{align*}
So, we apply $P(S_N = 4) = \sum_{n=1}^{4} P(S_N = 4 \mid N = n)\,P(N = n)$
and we have,
\begin{align*}
P(S_N = 4)
    &= \bigl(e^{-3}\cdot 3\bigr)(0.25)
     + \Bigl(e^{-3}\cdot\tfrac{9}{2}\Bigr)
       \bigl((0.4)^{2} + 2(0.05\cdot 0.1)\bigr) \\
    &\quad
     + \Bigl(e^{-3}\cdot\tfrac{27}{6}\Bigr)3\,(0.05)^{2}(0.4)
     + \Bigl(e^{-3}\cdot\tfrac{81}{24}\Bigr)(0.05)^{4} \\[4pt]
    &= e^{-3}\!\left[
         (3\cdot 0.25)
         + \frac{9}{2}\bigl((0.4)^{2} + 2(0.05\cdot 0.1)\bigr)
         + \frac{27}{6}\cdot 3\,(0.05)^{2}(0.4)
         + \frac{81}{24}\,(0.05)^{4}
       \right] \\
    &= e^{-3} \cdot 1.528521094.
\end{align*}
With this intuitive example in mind, we now calculate this same compound random
variable using the formula from Corollary~1.
...
P(S_N = 4)
    ...
    &= 1.528521094\,e^{-3}.
\end{align*}
Note that this result agrees with the value $e^{-3} \cdot 1.528521094$ obtained above
via the law of total probability, confirming the two methods are consistent.
\end{example}

%% ------ Example 2: Negative Binomial ------
\begin{example}[N in the Negative Binomial Distribution]\label{ex:negbin}
\addcontentsline{toc}{subsection}{Example~\theexample: N in the Negative Binomial Distribution}
...
\addcontentsline{toc}{subsection}{Example~\theexample: N in the Binomial Distribution}
...
\addcontentsline{toc}{subsection}{Example~\theexample: N in a Finite Mixture of Two Poisson Distributions}
...
\begin{proposition}\label{prop:not-panjer}
The probability mass function
$P(N = n) = \beta_1 \cdot \dfrac{e^{-\lambda_1}\lambda_1^n}{n!}
           + \beta_2 \cdot \dfrac{e^{-\lambda_2}\lambda_2^n}{n!}$
is not a member of the Panjer class.
\end{proposition}
...
$P(N = n) = \beta_1 \cdot \dfrac{e^{-\lambda_1}\lambda_1^n}{n!}
           + \beta_2 \cdot \dfrac{e^{-\lambda_2}\lambda_2^n}{n!}$
is a member of the Panjer class.  Therefore, the function must satisfy the Panjer
recurrence relation,
\[
    P(N = n) = \left(a + \frac{b}{n}\right) \cdot P(N = n-1),
    \qquad n \in \mathbb{Z},\; n > 0,\; a, b \text{ are constants.}
\]
So,
\[
    \frac{P(N = n)}{P(N = n-1)} = \left(a + \frac{b}{n}\right),
    \qquad \forall\, n \in \mathbb{Z} \text{ where } n > 0,\; a, b \text{ are constants.}
\]
Then,
\[
    \frac{\displaystyle\beta_1 \cdot \frac{e^{-\lambda_1}\lambda_1^n}{n!}
        + \beta_2 \cdot \frac{e^{-\lambda_2}\lambda_2^n}{n!}}%
     {\displaystyle\beta_1 \cdot \frac{e^{-\lambda_1}\lambda_1^{n-1}}{(n-1)!}
        + \beta_2 \cdot \frac{e^{-\lambda_2}\lambda_2^{n-1}}{(n-1)!}}
    = \left(a + \frac{b}{n}\right),
    \qquad \forall\, n \in \mathbb{Z} \text{ where } n > 0,\; a, b \text{ are constants.}
\]
We evaluate the ratio for $n = 1$, $n = 2$, $n = 3$, and $n = 4$.
Let $\beta_1 = 0.5$, $\beta_2 = 0.5$, $\lambda_1 = 1$, $\lambda_2 = 2$.

When $n = 1$ we have,
\begin{align*}
    &= \frac{e^{-1} + e^{-2} \cdot 2}{e^{-1} + e^{-2}}
     = (a + b),
     \qquad \text{where } a, b \text{ are constants,}\\
    &= \frac{e + 2}{e + 1} = a + b,
     \qquad \text{where } a, b \text{ are constants.}
\end{align*}
When $n = 2$ we have,
\begin{align*}
    &= \frac{\dfrac{e^{-1}}{2} + \dfrac{e^{-2} \cdot 4}{2}}{e^{-1} + e^{-2} \cdot 2}
     = \frac{2a + b}{2},
     \qquad \text{where } a, b \text{ are constants,}\\
    &= \frac{1}{2} \cdot \frac{e + 4}{e + 2}
     = \frac{2a + b}{2},
     \qquad \text{where } a, b \text{ are constants,}\\
    &\frac{e + 4}{e + 2} = 2a + b,
     \qquad \text{where } a, b \text{ are constants.}
...
When $n = 3$ we have,
\begin{align*}
    &= \frac{\dfrac{e^{-1}}{6} + \dfrac{e^{-2} \cdot 8}{6}}{\dfrac{e^{-1}}{2} + \dfrac{e^{-2} \cdot 4}{2}}
     = \frac{3a + b}{3},
     \qquad \text{where } a, b \text{ are constants,}\\
    &\frac{1}{3} \cdot \frac{e + 8}{e + 4} = \frac{3a + b}{3},
     \qquad \text{where } a, b \text{ are constants,}\\
    &\frac{e + 8}{e + 4} = 3a + b,
     \qquad \text{where } a, b \text{ are constants.}
\end{align*}
...
And by substitution,
\begin{align*}
    a &= \frac{e + 2}{e + 1} - \left(\frac{e + 4}{e + 2} - 2a\right)\\
    a &= \frac{e + 2}{e + 1} - \left(\frac{e + 4}{e + 2}\right) + 2a\\
    a &= \frac{e + 4}{e + 2} - \frac{e + 2}{e + 1}
       = \frac{(e^2 + 5e + 4) - (e^2 + 4e + 4)}{e^2 + 3e + 2}\\
    a &= \frac{e}{e^2 + 3e + 2}.
\end{align*}
And also by substitution,
\begin{align*}
    b &= \frac{e + 4}{e + 2} - 2\!\left(\frac{e}{e^2 + 3e + 2}\right)\\
    b &= \frac{e + 4}{e + 2} - \frac{2e}{(e + 1)(e + 2)}\\
    b &= \frac{(e + 1)(e + 4) - 2e}{(e + 1)(e + 2)}\\
    b &= \frac{e^2 + 3e + 4}{(e + 1)(e + 2)}.
\end{align*}
So we must have,
\[
    \frac{e + 8}{e + 4} = 3a + b.
...
\begin{align*}
    \frac{e + 8}{e + 4}
        &= \left(3 \cdot \frac{e}{e^2 + 3e + 2}
           + \frac{e^2 + 3e + 4}{(e + 1)(e + 2)}\right)\\
    \frac{e + 8}{e + 4}
        &= \frac{e^2 + 6e + 4}{e^2 + 3e + 2}\\
    e^3 + 11e^2 + 26e + 16
        &= e^3 + 10e^2 + 28e + 16\\
    e^2 &= 2e.
\end{align*}
This implies $e = 2$, which contradicts the known irrational value of $e$.
Therefore, the Panjer recurrence relation does not hold, and
$P(N = n) = \beta_1 \cdot \dfrac{e^{-\lambda_1}\lambda_1^n}{n!}
           + \beta_2 \cdot \dfrac{e^{-\lambda_2}\lambda_2^n}{n!}$
is not a member of the Panjer class.
\qedhere
...
Having shown that $P(N = n)$ is not in the Panjer class and is therefore not eligible for
Panjer recursion, we will seek to apply the Pek\"{o}z and Ross method.
...
\addcontentsline{toc}{subsection}{Example~\theexample: Revisiting Example~\protect\ref{ex:poisson}}
...
\addcontentsline{toc}{subsection}{Example~\theexample: Revisiting Example~\protect\ref{ex:poisson-mix}}
...
Let $N^{(s)}$ be a mixed random variable of Poisson, Binomial, or Negative Binomial
distribution at recursion level $s$ with probability mass function:
\[
    P^{(s)}(N = n) = \sum_{j=1}^{t} \beta_j^{(s)}\, P_{r_j}^{(s+1)}(N = n), \qquad n \geq 0,
\]
where $\sum_{j=1}^{t}\beta_j^{(s)} = 1$.  Let $M^{(s)}$ be the random variable satisfying the
compound identity condition associated with $N^{(s)}$.  We claim that for all $s \geq 0$:
\begin{enumerate}
\item $E[N^{(s)}] = \displaystyle\sum_{j=1}^{t} \beta_j^{(s)}\, E[N_j^{(s)}].$
\item The distribution of $M^{(s)} - 1$ is identical to the distribution of $N$ in $P^{(s+1)}(N=n)$, also denoted $N^{(s+1)}$,
...
\begin{proof}[Proof (by induction)]
\textit{Base Case} $(s = 0)$: For $s = 0$, the weights are $\beta_j^{(0)}$.  First, we
compute the expected value of $E[N^{(0)}]$:
\begin{align*}
E[N^{(0)}]
    &= \sum_{n=0}^{\infty} n \cdot \left[\sum_{j=1}^{t}\beta_j^{(0)}\,P_{r_j}^{(0)}(N=n)\right]
...
P(M^{(0)} - 1 = n)
    &= P(M^{(0)} = n+1)
     = \frac{(n+1)\,P^{(0)}(N = n+1)}{E[N^{(0)}]} \\
    &= \frac{1}{E[N^{(0)}]}
       \sum_{j=1}^{t}\beta_j^{(0)}\!\left[(n+1)\cdot P_{r_j}^{(0)}(N = n+1)\right].
\end{align*}
For each component distribution, the compound identity shifts the parameter by one level of
recursion. So given the expression, 
\[
    (n+1)\,P_r^{(s)}(N = n+1) = E[N_{j}^{(s)}]\, P_{r'}^{(s+1)}(N = n)
\]
For the negative binomial distribution case $r'$ becomes $r+1$, in the binomial distribution case $r'$ becomes $r-1$ while,
for the Poisson distribution the parameter is unchanged, that is,  $r'= r$.

Substituting the appropriate identity for each component, we have
\begin{align*}
P(M^{(0)} - 1 = n)
    &= \frac{1}{E[N^{(0)}]}
       \sum_{j=1}^{t}\beta_j^{(0)}\, E[N_j^{(0)}] \cdot P_{r_j}^{(1)}(N = n) \\
    &= \sum_{j=1}^{t}
       \frac{\beta_j^{(0)}\, E[N_j^{(0)}]}{E[N^{(0)}]} \cdot P_{r_j}^{(1)}(N = n),
\end{align*}
where $P_{r_j}^{(1)}(N = n)$ denotes the $j$th component advanced to level $1$ (parameter
$r_j + 1$ for negative binomial, $r_j - 1$ for binomial, and unchanged for Poisson).
Defining $\beta_j^{(1)} = \dfrac{\beta_j^{(0)}\, E[N_j^{(0)}]}{E[N^{(0)}]}$, this is exactly
the mixture $P^{(1)}(N = n)$, and the new weights sum to one since
$\sum_{j=1}^{t}\beta_j^{(1)} = \frac{1}{E[N^{(0)}]}\sum_{j=1}^{t}\beta_j^{(0)}\,E[N_j^{(0)}] = 1$.
Thus, the claim holds for $s = 0$.

\medskip
\textit{Inductive Step}: Assume the statement is true for $s = k$.  That is, the distribution
of $M^{(k)} - 1$ is equivalent to the distribution of $N^{(k+1)}$ and $E[N^{(k)}]$ follows the
claim.  We need to show the statement is true for $s = k + 1$.  By the inductive hypothesis,
the distribution of $N^{(k+1)}$ has the probability mass function of the mixture
$\sum_{j=1}^{t}\beta_j^{(k+1)}\,P_{r_j}(N^{(k+1)} = n)$, so we have,
\[
    E[N^{(k+1)}]
    = \sum_{n=0}^{\infty} n \sum_{j=1}^{t}\beta_j^{(k+1)}\,P_{r_j}^{(k+1)}(N = n)
    = \sum_{j=1}^{t}\beta_j^{(k+1)}\,E[N_j^{(k+1)}],
\]
which confirms Claim~1 for $s = k+1$.  Next, we determine the distribution of
$M^{(k+1)} - 1$.  We have,
\begin{align*}
P(M^{(k+1)} - 1 = n)
    &= P(M^{(k+1)} = n+1)
     = \frac{(n+1)\,P^{(k+1)}(N = n+1)}{E[N^{(k+1)}]} \\
    &= \frac{1}{E[N^{(k+1)}]}
       \sum_{j=1}^{t}\beta_j^{(k+1)}\!\left[(n+1)\cdot P_{r_j}(N^{(k+1)} = n+1)\right].
\end{align*}
Applying the same per-component shift identity used in the base case, now advancing each
component from level $k+1$ to level $k+2$, we have
\begin{align*}
P(M^{(k+1)} - 1 = n)
    &= \frac{1}{E[N^{(k+1)}]}
       \sum_{j=1}^{t}\beta_j^{(k+1)}\, E[N_j^{(k+1)}] \cdot P_{r_j}^{(k+2)}(N = n) \\
    &= \sum_{j=1}^{t}
       \frac{\beta_j^{(k+1)}\, E[N_j^{(k+1)}]}{E[N^{(k+1)}]} \cdot P_{r_j}^{(k+2)}(N = n).
\end{align*}
This expression is exactly the PMF for $N^{(k+2)}$ with updated weights
$\beta_j^{(k+2)} = \dfrac{\beta_j^{(k+1)}\, E[N_j^{(k+1)}]}{E[N^{(k+1)}]}$, as desired.
...
\]
where $E[N^{(s)}] = \sum_{j=1}^{t}\beta_j^{(s)}\,E[N_j^{(s)}]$ and the computation of
...
\addcontentsline{toc}{subsection}{Example~\theexample: N in a Finite Mixture of Binomial Distributions}
Consider a discrete mixture distribution where the event is generated by one of two random
...
\label{ex:negbin-mix}
\addcontentsline{toc}{subsection}{Example~\theexample: N in a Finite Mixture of Negative Binomial Distributions}
Consider a discrete mixture distribution where the event is generated by one of two random
...
\addcontentsline{toc}{subsection}{Example~\theexample: N in a Finite Mixture of Poisson, Binomial and Negative Binomial Distributions}
...
Computational analysis was conducted to compare accuracy of results, number of
evaluations required, and memory used when utilizing the recursive algorithm defined in
Corollary~3 as compared to the convolutional Law of Total Probability method discussed in
the first example.

\subsection{Accuracy}

The computation results of both methods agreed to a level of precision up to the precision
of the underlying numeric data structure of the programming language ($\cong 10^{-10}$).
This level of accuracy was consistent throughout all the examples. The results were further
validated against a Monte Carlo simulation of 500,000 draws simulating the results of each
of the examples at various levels of $K$. All results agreed with the Monte Carlo simulated
values within less than 6\% relative error. Therefore, the computational analysis provided
clear validation of the accuracy of the method.

\subsection{Memory Usage}

The implementation of both methods requires the use of memoization. Without memoization
the recursion tree requires both methods to recompute every subproblem and the number of
evaluations grows exponentially. This would make computation untenable at levels of
$k \geq 20$. Memory usage was measured as the number of evaluations cached in the process
of computing a result. The recursive method required an $O(k^2)$ level of storage to
compute a result. The recursive method's evaluations accumulate in memory and are never
evicted. The Law of Total Probability method required an $O(k)$ level of storage as only
the current convolution slice is held in memory and previous slices are evicted. The Law
of Total Probability method had a clear advantage over the recursive method in memory
usage.

\subsection{Evaluation Efficiency}

Evaluation efficiency was measured between methods by counting the number of
convolution-sum multiplications. For the recursive method this is the count of
$(i \cdot \alpha_i \cdot P_{s+1})$ evaluations computed to arrive at a result. For the Law
of Total Probability method this is the count of $(\alpha_i \cdot f_{n-1}(j - i))$
evaluations computed to arrive at a result. Both methods resulted in an evaluation
efficiency of $O(k^2)$ level of complexity. The recursive method has a slight advantage at
small values of $k$ but the number of evaluations converges to 1 to 1 by $k = 200$.

Overall, the new recursive algorithm defined in Corollary~3 does not offer a computational
advantage over the pre-existing Law of Total Probability convolutional method. In fact, it
is at a computational disadvantage in that it requires significantly more memory to compute
its results efficiently. The value of the recursive method lies in the algebraic result
itself. The closed-form weight update and the proof that the framework extends to finite
mixtures is not in runtime or memory efficiency.

The implementation and its output can be found at
\url{https://github.com/robrighter/masters-thesis/tree/main/implementation}.
...
The primary contribution of this thesis is the extension of the Pek\"{o}z-Ross identity to the
case of finite mixtures of $(a, b, 0)$ class distributions. Corollary~3 derives a more general
form of the recursive expression for calculating compound random variables. Furthermore,
this corollary provides a closed-form formula for the recursive mixture weights, bypassing
the need for nested recursion. This allows the framework to calculate the single-distribution
examples in the Pek\"{o}z and Ross 2004 paper, as well as complex counting distributions
made up of finite mixtures of the $(a, b, 0)$ class.

Panjer (1981) and Sundt \& Jewell (1981) established the $(a, b, 0)$ class using direct
recurrence, and Willmot (1993) extended the recurrence to continuous mixtures. However,
those continuous mixtures ultimately collapse into a single marginal distribution (like the
Negative Binomial). The finite mixture counting distributions introduced in this thesis
maintain the structural integrity of distinct subsets, such as higher-risk versus lower-risk
insurance groups.

The technique offers a unified recursive method regardless of which single $(a, b, 0)$ class
counting distribution is chosen or finite mixtures of $(a, b, 0)$ class distributions. This
reduces some of the complexity associated with manual computation of probabilities. While
the method does not introduce a computational advantage over existing convolutional
techniques, computation times associated with this recursive method are at parity with
existing methods.

\subsection{Applications and Future Work}

The primary value of the finite mixture framework lies in its ability to model genuine
population subgroups in the counting distribution of compound random variables. While
prior work often assumes a homogeneous counting distribution for mathematical
convenience, real-world data, particularly in insurance and telecommunications, is rarely
so uniform.

A clear application is aggregate loss modeling. For example, a car insurance portfolio is
naturally composed of distinct subgroups. These might include a `low risk group' modeled
with the Poisson distribution using a small $\lambda$, and a `high risk group' modeled with
a Poisson distribution using a large $\lambda$. This method provides a recursive tool for
calculating the risk probability while maintaining that population segmentation throughout
the calculations. The same pattern is applicable in other domains such as healthcare,
where one might segment patient populations, or telecommunications networks, where one
might segment traffic groups. In each of these cases, modeling the structure of the
underlying subgroups with finite mixtures can yield a more accurate probability model.

There are several promising possibilities for future research in this area. Because the
finite mixture framework is currently limited to $(a, b, 0)$ class counting distributions, a
natural next step would be extending the technique to the $(a, b, 1)$ class. Furthermore,
this research did not evaluate numerical resilience at extremely high values of $k$ or with
a massive number of mixture components. It is entirely possible that this recursive
technique offers a numerical stability advantage over conventional convolution methods at
larger scales.
...
\bibitem{ross2010}
S. M. Ross, \emph{Introduction to Probability Models}, 10th ed., Academic Press,
Amsterdam, 2010.

\bibitem{pekoz-ross}
E. Pek\"{o}z and S. M. Ross, ``Compound Random Variables,'' manuscript, School of
Management, Boston University, and Epstein Department of Industrial and Systems
Engineering, University of Southern California.


\bibitem{sundt-jewell1981}
B. Sundt and W. S. Jewell, ``Further Results on Recursive Evaluation of Compound
Distributions,'' \emph{ASTIN Bulletin}, vol.~12, pp.~27--39, 1981.

\bibitem{panjer1981}
H. H. Panjer, ``Recursive Evaluation of a Family of Compound Distributions,''
\emph{ASTIN Bulletin}, vol.~12, 1981.

\bibitem{willmot1993}
G. E. Willmot, ``On Recursive Evaluation of Mixed Poisson Probabilities and Related
Quantities,'' \emph{Scandinavian Actuarial Journal}, vol.~2, pp.~114--133, 1993.

\bibitem{klugman2012}
S. A. Klugman, H. H. Panjer, and G. E. Willmot, \emph{Loss Models: From Data to
Decisions}, 4th ed., Wiley, Hoboken, NJ, 2012.

\end{thebibliography}

\newpage
\section*{Biographical Statement}
Robert Righter is a graduate student in the Department of Mathematics at
Shawnee State University. He received a bachelor's degree in computer science from
Covenant College in Lookout Mountain, GA and a bachelor's degree in Mathematics from Thomas Edison State
University in Trenton, NJ. He is currently a Vice President of Software Engineering at Salesforce.
He is primarily interested in the fields of Mathematical Analysis and Probability.
