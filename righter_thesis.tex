...
recursion.  For the negative binomial distribution $r'$ becomes $r+1$,
\[
    (n+1)\,P_r^{(s)}(N = n+1) = E[N_{j}^{(s)}]\, P_{r'}^{(s+1)}(N = n);
\]
for the binomial distribution $r'$ becomes $r-1$,
\[
    (n+1)\,P_r^{(s)}(N = n+1) = E[N_{j}^{(s)}]\, P_{r'}^{(s+1)}(N = n);
\]
and for the Poisson distribution the parameter is unchanged,  $r'= r$, ,
\[
    (n+1)\,P_r^{(s)}(N = n+1) = E[N_{j}^{(s)}]\, P_{r'}^{(s+1)}(N = n).
\]
...
$\sum_{j=1}^{t}\beta_j^{(k+1)}\,P_{r_j}(N^{(k+1)} = n)$, so by linearity of expectation,
\[
    E[N^{(k+1)}]
