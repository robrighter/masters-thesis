...
\begin{align*}
E[N^{(0)}]
    &= \sum_{n=0}^{\infty} n \cdot \left[\sum_{j=1}^{t}\beta_j^{(0)}\,P_{r_j}^{(0)}(n)\right]
...
\end{align*}
where $P_{r_j}(N^{(1)} = n)$ denotes the $j$th component advanced to level $1$ (parameter
...
\textit{Inductive Step}: Assume the statement is true for $s = k$.  That is, $M^{(k)} - 1$
is equivalent to $N^{(k+1)}$ and $E[N^{(k)}]$ follows the claim.  We need to show the
statement is true for $s = k + 1$.  By the inductive hypothesis, $N^{(k+1)}$ is the mixture
